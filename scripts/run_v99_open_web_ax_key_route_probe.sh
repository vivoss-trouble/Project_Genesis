...
MAPPER_BIN="${GENESIS_V99_MAPPER_BIN:-/tmp/genesis_v99_open_web_shadow_map}"
FOCUS_BIN="${GENESIS_V99_FOCUS_BIN:-/tmp/genesis_v99_ax_focus_browser_window}"
OS_SOCKET="${GENESIS_V99_OS_SOCKET:-/tmp/genesis_os_driver_v99.sock}"
DRIVER_LOG="${GENESIS_V99_DRIVER_LOG:-/tmp/genesis_os_driver_v99.log}"
OUTPUT_DIR="${GENESIS_V99_OUTPUT_DIR:-/tmp/genesis_v99_open_web_ax_key_route_probe}"
RESULTS_LOG="$OUTPUT_DIR/results.jsonl"
TARGET_URL="${GENESIS_V99_URL:-https://doc.rust-lang.org/book/}"
WINDOW_TITLE="${GENESIS_V99_WINDOW_TITLE:-The Rust Programming Language}"
BROWSER_APP="${GENESIS_V99_BROWSER_APP:-Safari}"
BROWSER_BUNDLE_ID="${GENESIS_V99_BROWSER_BUNDLE_ID:-com.apple.Safari}"
KEY_NAME="${GENESIS_V99_KEY:-page_down}"
POST_KEY_SETTLE_SEC="${GENESIS_V99_POST_KEY_SETTLE_SEC:-0.8}"
ARMED_TOKEN="GENESIS_V99_ARMED_OPEN_WEB_AX_KEY_ROUTE"
AUTO_KEY_TOKEN="GENESIS_V99_AUTO_KEY_ROUTE_PROBE"
DRIVER_PID=""
...
wait_for_socket() {
    ...
    echo "[v9.9] ERROR: timed out waiting for $socket_path" >&2
    exit 1
}
...
wait_for_target_url() {
    ...
    echo "[v9.9] ERROR: front browser URL did not settle on $url (current: $current)" >&2
    exit 1
}
...
with open(sys.argv[1], "r", encoding="utf-8") as handle:
    for raw in handle:
        ...
        if payload.get("event") == "open_web_shadow_map":
            event = payload
if event is None:
    raise SystemExit(f"[v9.9] missing map event in {sys.argv[1]}")
    ...
    with open(path, "r", encoding="utf-8") as handle:
        for raw in handle:
            ...
            if payload.get("event") == "open_web_shadow_map":
                event = payload
    if event is None:
        raise SystemExit(f"[v9.9] missing map event in {path}")
    return event
...
print(json.dumps({
    "event": "v99_ax_key_route_probe_result",
...
echo "========================================================================"
echo "Genesis v9.9 Open-Web AX Key Route Probe"
echo "========================================================================"
echo "[v9.9] URL: $TARGET_URL"
echo "[v9.9] Browser: $BROWSER_APP"
echo "[v9.9] Window title needle: $WINDOW_TITLE"
echo "[v9.9] Restricted key: $KEY_NAME"
...
ARMED=false
if [[ "${GENESIS_V99_ARMED_CONFIRM:-}" == "$ARMED_TOKEN" ]]; then
    ARMED=true
fi

open_target_url "$TARGET_URL"
wait_for_target_url "$TARGET_URL"
sleep "${GENESIS_V99_BROWSER_SETTLE_SEC:-2.5}"
BASE_URL="$(front_url)"
...
emit "$FOCUS_JSON"
sleep "${GENESIS_V99_POST_FOCUS_SETTLE_SEC:-0.6}"
FOCUS_URL="$(front_url)"
...
if [[ "$ARMED" == true ]]; then
    if [[ "${GENESIS_V99_AUTO_KEY_CONFIRM:-}" != "$AUTO_KEY_TOKEN" ]]; then
        echo "[v9.9] Armed AX key route probe requires GENESIS_V99_AUTO_KEY_CONFIRM=$AUTO_KEY_TOKEN" >&2
        exit 1
    fi
    echo "[v9.9] ARMED AX key route probe requested. It will post one bounded key event only."
    cargo run -p genesis-os-driver -- daemon --socket "$OS_SOCKET" \
        --armed --confirm GENESIS_OS_DRIVER_ARMED > "$DRIVER_LOG" 2>&1 &
else
    echo "[v9.9] Dry-run mode. Restricted key request is routed through unarmed os-driver only."
    cargo run -p genesis-os-driver -- daemon --socket "$OS_SOCKET" \
        > "$DRIVER_LOG" 2>&1 &
fi
DRIVER_PID=$!
...
PROBE_JSON="$(roundtrip_os_driver '{"request_id":"probe-v99-open-web-ax-key-route","act":"probe"}')"
...
if not payload.get("probe", {}).get("accessibility_trusted"):
    raise SystemExit("[v9.9] Accessibility is not trusted; refusing armed key route probe")
PY
fi
...
key_name = sys.argv[1]
print(json.dumps({
    "request_id": "key-v99-open-web-ax-key-route",
    "action_id": f"act-v99-open-web-ax-key-{key_name}",
    "act": "key_press",
    "key": key_name,
}, sort_keys=True))
PY
...
PY
    ...
    if [[ -n "$ABORT_JSON" ]]; then
        emit "$ABORT_JSON"
        echo "[v9.9] ERROR: armed restricted key changed URL; fail-fast stop engaged" >&2
        exit 2
    fi
fi
...
if result.get("url_changed_after_key"):
    raise SystemExit(f"[v9.9] restricted key changed URL: {result}")
if not armed and result.get("key_posted") is not False:
    raise SystemExit(f"[v9.9] dry-run leaked physical key input: {result}")

print(json.dumps({
    "event": "v99_open_web_ax_key_route_probe_summary",
    "armed": armed,
    "requested_key": result.get("requested_key"),
...
echo "========================================================================"
echo "Genesis v9.9 open-web AX key route probe complete"
echo "========================================================================"
