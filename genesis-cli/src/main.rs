...
use lazarus_synthesizer::{
    BehaviorCase, OpenAiOracleAdapter, SynthesisInput, SynthesisVerdict, SynthesizerConfig,
...
fn run_corpus_report(args: &[String]) -> Result<(), String> {
    if args.len() != 2 {
        return Err(
...
fn run_synthesis_smoke(args: &[String]) -> Result<(), String> {
    if args.len() != 4 {
        return Err(
...
fn run_corpus_ingest(args: &[String]) -> Result<(), String> {
    if args.len() != 2 {
        return Err(
...
fn load_valid_snapshots_for_method(
    input: &Path,
    business_method: &str,
...
fn derive_row_count_behavior_case(snapshot: &StateSnapshot) -> Result<BehaviorCase, String> {
